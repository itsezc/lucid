...
import { IModel, IBasicModel, Model } from "../model.js";
import { Simplify, OfArray } from "../utilities/helper.types.js";

export type Basic<T> = T extends object
	? {
			[P in keyof T]: NonNullable<T[P]> extends Model
				? string
				: NonNullable<T[P]> extends Model[]
				? string[]
				: T[P] extends Date
				? T[P]
...
export type OnlyModelProps<T extends IModel> = { [P in keyof T as NonNullable<T[P]> extends Model<any> | Model<any>[] ? P : never]: T[P] };
export type OnlyRecordProps<T extends IModel> = { [P in keyof T as NonNullable<T[P]> extends Model<false> | Model<false>[] ? P : never]: T[P] };
export type OnlyRelationalProps<T extends IModel> = { [P in keyof T as NonNullable<T[P]> extends Model<true> | Model<true>[] ? P : never]: T[P] };

export type SubsetModel<M extends PartialId<IBasicModel>> = ToBasicModel<{ [P in keyof M as M[P] extends Function ? never : P]: M[P] }>;
export type SubsetModelOrBasic<M extends PartialId<IBasicModel>> =
	| ToBasicModel<{
			[P in keyof M as M[P] extends Function ? never : P]: M[P];
	  }>
	| (Basic<SubsetModel<M>> & IBasicModel);
