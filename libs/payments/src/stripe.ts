...
export async function sync({ providers, plans }: TPricingProps) {
	if (providers.stripe) {
		...
		plans.map(async ({ name, description, mode, recurring, features, ...props }) => {
			Object.entries(features).map(async (feature) => {
				...
				const currency_options: Record<string, Stripe.PriceCreateParams.CurrencyOptions> = {};
				...
				const tiered = primaryCurrency.tiers && primaryCurrency.tiers.length > 0;
